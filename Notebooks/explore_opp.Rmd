...
pop_stop_plot <- ggplot(data = stops_by_pop) +
  ...
  # actually na maybe cluttered
  # geom_text(
  #   aes(x = race5, y = pct + 3, label = pct_label, group = statistic),
  #   position = "fill",
...
```{r outcome_data, echo=F, warning=F}
...
outcome_plot <- function(df, city_name){
  df %>%
    filter(
      # we're doing a seperate plot by city
      city == city_name, 
      # dropping citations for plots but maybe asians cited at higher rates. 
      # come back to this
      outcome != "Citation"
      ) %>%
    ggplot() +
  ...
  geom_text(
    aes(x = race5, y = pct + 3, label = pct_label),
    color = "black", fontface = "bold"
  ) +
  theme_bw() +
  theme(
...
# plots!
outcome_plot(outcomes, "Oakland")
outcome_plot(outcomes, "San Francisco")
outcome_plot(outcomes, "San Jose")
```

```{r models, include=F}
# basically a bunch of dummies to save time re-labeling plots lol
stops_model_data <- select(stops, city, arrest_made, search_conducted) %>%
  ...
  bind_cols(willbprocessed::factor_to_dummies(factor(stops$type))) %>%
  rename("Pedestrian" = "pedestrian")

outcome_models <- function(df, city_name){
  # just the city in question
  tmp <- filter(df, city == city_name)
  # will be the same independent vars so ...
  out_models <- map(c("arrest_made", "search_conducted"), function(var_name){
    glm(
      # change to black, whit hisp + male + summer + winter
      tmp[[var_name]] ~ Black + Hispanic + White + Male + Summer + Winter,
      data = tmp,
...
oak_models <- outcome_models(stops_model_data, "Oakland")
sf_models <- outcome_models(stops_model_data, "San Francisco")
# sj instead of gender use type
# sj_models <- outcome_models(stops, "San Jose")
sj_models <- map(c("arrest_made", "search_conducted"), function(var_name){
  tmp <- filter(stops_model_data, city == "San Jose")
  glm(
      # no gender buuuut stop type for most people so we'll do that
      tmp[[var_name]] ~ Black + Hispanic + White + Summer + Winter + Pedestrian,
      data = tmp,
      family = quasibinomial(link = "logit")
      )})

names(sj_models) <- c("Arrest", "Search")

```

```{r coef_plot_fxn, include=F}

outcome_coef_plot <- function(city_model, city_name){

  coefplot::multiplot(city_model,
                      innerCI = 1, 
                      outerCI = 2,
                      sort = "magnitude", 
                      legend.reverse = T,
...
```{r coef_plots, echo=F, fig.width = 11, fig.height=3, message=F}

outcome_coef_plot(oak_models, "Oakland")
outcome_coef_plot(sf_models, "San Francisco")
outcome_coef_plot(sj_models, "San Jose")
