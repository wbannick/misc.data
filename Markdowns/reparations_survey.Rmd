...
# the plot
ggplot(data = reparations_breakdown, aes(x="", fill = reparations, y = pct)) + 
...
rep3_race3 <- df %>%
    ...
    pivot_longer(cols = matches(" |Support|Oppose"), names_to = "reparations", values_to = "value") %>%
    mutate(reparations = factor(reparations, levels = c("Support", "Oppose", "Not sure"))) %>%
    srvyr::as_survey(weights = weight, ids = respondent_id) %>%
...
# 
# rep_race3_plot <- ggplot(data = reparations_race3_data) + 
#   #to do convert this and get conf intervals..
#   geom_bar(stat = "identity", aes(x = race3, y = pct, fill = reparations), color = "black") +
#   coord_flip() +
#   #facet_wrap(~survey, ncol = 1, scales = "free_y") +
#   geom_text(
#     aes(x = race3, y = pct, label = pct_label, group = reparations),
#     position = position_stack(vjust = 0.5),
